...
import { SIGNER_NUM, SIGNER_VERIFY_NUM } from '../../../src/bcp01/contract-proto/nft.proto'
...
for (let i = 0; i < signerNum; i++) {
  ...
  satotxSigners.push(new MockSatotxSigner(BN.fromString(p, 10), BN.fromString(q, 10)))
}
...
function expectFeeb(tx: bsv.Transaction, feeb: number) {
  ...
  expect(isValid, `feeb should be ${feeb} but finally is ${finalFeeb}`).to.be.true
}
describe('BCP01-NonFungibleToken Test', () => {
  describe('basic test ', () => {
    ...
    it('issue should be ok', async () => {
      let _res1 = await nft.issue({
        ...
        metaTxId: 'a9a6634a67e0785f33efa1ff91b12888be28bac28878c28687055f69be2adac1',
      })
  ...
  describe('reach totalSupply test ', () => {
    ...
    it('issue #1 NFT should fail for reaching the totalSupply', async () => {
      try {
        ...
        expect(error.code == ErrCode.EC_FIXED_TOKEN_SUPPLY, 'it should reach the totalSupply').to.be
          .true
      }
    })
  })
  ...
  describe('sell test ', () => {
    ...
    it('put on sell nft #0 should be ok', async () => {
      ...
      let pkh = bsv.crypto.Hash.sha256ripemd160(sellTx.outputs[0].script.toBuffer())
