...
describe('Test nft contract unlock In Javascript', () => {
    ...
    it('should success when unlock from genesis', () => {
        ...
        const newGenesisScriptBuf = NftProto.getNewGenesisScript(genesis.lockingScript.toBuffer(), sensibleID, BigInt(0))
        const genesisHash = mvc.crypto.Hash.sha256ripemd160(newGenesisScriptBuf)

        transferNft(10, 5, {genesisHash, prevScriptBuf: genesis.lockingScript.toBuffer(), genesisScriptHex: genesis.lockingScript.toBuffer().toString('hex')})
    })
