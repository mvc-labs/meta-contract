...
const inputSatoshis2 = inputSatoshis - 100
...
const genesisTxid = Buffer.concat([Common.getTxIdBuf(dummyTxId), Common.getUInt32Buf(0)])
let tokenID
...
function createUnlockContractCheck(
  ...
  return {
    ...
    amountCheckTx: tx,
    amountCheckScriptData: Common.buildOpReturnData(unlockContractCheckData),
  }
}
...
function unlockTransferCheck(
  ...
  for (let i = 0; i < tokenTxArray.length; i++) {
    ...
    tokenTxHeaderArray = Buffer.concat([
      tokenTxHeaderArray,
      Buffer.from(proof.txHeader.toHex(), 'hex'),
    ])
    ...
    tokenTxSatoshisBytesArray = Buffer.concat([
      ...
      Buffer.from(proof.satoshiBytes.toHex(), 'hex'),
    ])
  }
...
function verifyTokenTransfer(
  ...
  for (let i = 0; i < nTokenInputs; i++) {
    ...
    const sigHex = toHex(
      signTx(tx, privateKey, tokenInstance[i].lockingScript, inputSatoshis, i, sigtype)
    )
    unlockTokenContract(
      tx,
      prevouts,
...
function unlockTokenContract(
  ...
  const amountCheckTxOutputProofInfo = new TxOutputProof(
    Common.getTxOutputProof(amountCheckTx, amountCheckOutputIndex)
  )
  ...
  const prevTokenTxOutputProofInfo = new TxOutputProof(
    Common.getTxOutputProof(prevTokenTx, prevTokenOutputIndex)
  )

  let contractTxOutputProof = new TxOutputProof(Common.getEmptyTxOutputProof())
  if (op == TokenProto.OP_UNLOCK_FROM_CONTRACT) {
    contractTxOutputProof = new TxOutputProof(
      Common.getTxOutputProof(contractTx, contractOutputIndex)
    )
  }
...
function unlockFromContract(
  ...
  const { amountCheck, amountCheckTx, amountCheckScriptData } = createUnlockContractCheck(
    tokenInputIndexArray,
    nTokenOutputs,
    outputTokenArray,
    outputTokenAddress,
  ...
  for (let i = 0; i < nTokenOutputs; i++) {
    const tokenScriptBuffer = TokenProto.getNewTokenScript(
      tokenScript.toBuffer(),
      outputTokenAddress[i],
      outputTokenArray[i]
    )
...
function unlockFromGenesis(options: any = {}) {
  ...
  const { amountCheck, amountCheckTx, amountCheckScriptData } = createTransferCheckContract(
    1,
    1,
    [tokenAmount],
    tokenID,
  ...
  let sigHex = toHex(signTx(tx, privateKey, tx.inputs[0].output.script, inputSatoshis, 0, sigtype))
  console.log({
    pubKeyHex,
    sig: sigHex,
    pk: privateKey.toString(),
    sigtype,
    sats: tx.inputs[0].output.satoshis,
    sats2: inputSatoshis,
  })
