...
export async function getRabinDataEmpty(signers: SatotxSigner[], signerSelecteds: number[]) {
  let rabinMsg: Bytes
  ...
  rabinPubKeyIndexArray.forEach((signerIndex) => {
    rabinPubKeyVerifyArray.push(new Int(signers[signerIndex].satotxPubKey.toString(10)))
  })
  return {
    rabinData: {
      rabinMsg,
...
export async function getRabinData(
  ...
  if (!rabinUtxo) {
    ...
    for (let j = 0; j < sigReqArray.length; j++) {
      ...
      rabinSigArray.push(new Int(BN.fromString(sigInfo.sigBE, 16).toString(10)))
...
export async function getRabinDatas(
  ...
  for (let i = 0; i < sigReqArray.length; i++) {
    for (let j = 0; j < sigReqArray[i].length; j++) {
      ...
      checkRabinData.rabinSigArray.push(new Int(BN.fromString(sigInfo.byTxSigBE, 16).toString(10)))
...
export async function selectSigners(
  ...
  for (let i = 0; i < _signerConfigs.length; i++) {
    ...
    let ret = new Promise(
      (resolve: ({ url, idx }: { url: string; idx: number }) => void, reject) => {
  ...
  let getSelected = new Promise((resolve: (selected: number[]) => void, reject) => {
    let signerSelecteds = []
    let hasResolve = false
    let successCnt = 0
    let failedCnt = 0
