...
function getNftUnlockContractCheckCodeHashArray(): string[] {
  let contractArray = [
    NftUnlockContractCheckFactory.createContract(NFT_UNLOCK_CONTRACT_TYPE.OUT_3),
    NftUnlockContractCheckFactory.createContract(NFT_UNLOCK_CONTRACT_TYPE.OUT_6),
    NftUnlockContractCheckFactory.createContract(NFT_UNLOCK_CONTRACT_TYPE.OUT_10),
    NftUnlockContractCheckFactory.createContract(NFT_UNLOCK_CONTRACT_TYPE.OUT_20),
    NftUnlockContractCheckFactory.createContract(NFT_UNLOCK_CONTRACT_TYPE.OUT_100),
  ]
...
export class ContractUtil {
  ...
  public static init(config?: ContractConfig) {
    if (config) {
      this.unlockContractCodeHashArray = config.unlockContractCodeHashArray.map((v) => new Bytes(v))
      ...
      NftUnlockContractCheckFactory.unlockContractTypeInfos.forEach((v, idx) => {
        v.lockingScriptSize = NftUnlockContractCheckFactory.calLockingScriptSize(v.type)
      })
    }
