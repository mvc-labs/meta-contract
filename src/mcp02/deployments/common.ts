...
export const buildOutput = function (outputScriptBuf: Buffer, outputSatoshis: number) {
  return Buffer.concat([getUInt64Buf(outputSatoshis), writeVarint(outputScriptBuf)])
}
...
export const addOutput = function (tx: mvc.Transaction, lockingScript, outputSatoshis: number) {
  tx.addOutput(
